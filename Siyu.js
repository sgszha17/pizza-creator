...
const toppings = [{
  ...
  function displayToppingView(){
    ...
    toppings.forEach(({labelImage, name}) => {
        summaryElement = document.getElementById('orderSummary').innerText=totalPrice
        ...
        onToppingClick = enhancedOnToppingClicked(button,name);
        button.onclick = onToppingClick
  ...
  function removeStuffFromOrder(name){
    ...
    editSummary(false,price.medium)
  }

  function editSummary(plus,toppingPrice){
      ...
      summaryElement.innerText= "Total: $"+totalPrice.toFixed(2)
  }